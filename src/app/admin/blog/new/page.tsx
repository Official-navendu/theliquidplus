...
import { createBlogPostAction, getBlogCategoriesAction } from '@/features/catalog/actions/cms';
...
export default function AdminBlogNewPage() {
  ...
  React.useEffect(() => {
    async function loadCats() {
      const res = await getBlogCategoriesAction();
  ...
  const onSubmit = async (values: SafeAny) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.error(res.error?.message || 'Failed to save blog post');
      }
    } catch {
        ...
        <div className="grid grid-cols-1 items-start gap-6 lg:grid-cols-3">
          ...
          <div className="space-y-6 lg:col-span-1">
            <AdminCard className="space-y-4">
              ...
              >
                {submitting ? 'Creating...' : 'Save & Publish Article'}
              </button>
            </AdminCard>
          </div>
        </div>
